...
export default function StatCard({ label, value, unit, icon, trend, accentColor = 'border-primary', topBorder = true }: StatCardProps) {
  return (
    <div className={`bg-surface-container-low rounded-xl p-space-lg relative overflow-hidden ${topBorder ? `border-t-2 ${accentColor}` : ''}`}>
      <div className="flex justify-between items-start mb-4">
        <span className="text-sm font-medium text-on-surface-variant">{label}</span>
        <span className={`material-symbols-outlined text-xl text-on-surface-variant/50`}>{icon}</span>
      </div>
      <div className="flex items-baseline space-x-1">
        <span className="text-3xl font-mono font-bold text-on-surface">{value}</span>
